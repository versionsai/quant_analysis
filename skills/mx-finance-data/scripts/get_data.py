...
DEFAULT_MAX_RETRIES = max(1, int(os.environ.get("MX_FINANCE_DATA_MAX_RETRIES", "3") or "3"))
DEFAULT_RETRY_DELAY_SEC = max(0.5, float(os.environ.get("MX_FINANCE_DATA_RETRY_DELAY_SEC", "1.5") or "1.5"))
...
def _is_rate_limited(api_result: Any) -> bool:
    """
    判断是否触发接口限流。
    """
    if not isinstance(api_result, dict):
        return False
    message = _flatten_value(api_result.get("message") or "")
    status = api_result.get("status")
    code = api_result.get("code")
    return "操作过于频繁" in message or code in (429, 503, "429", "503") or status in (429, 503, "429", "503")
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            for attempt in range(1, DEFAULT_MAX_RETRIES + 1):
                ...
                data = resp.json()
                if not _is_rate_limited(data) or attempt >= DEFAULT_MAX_RETRIES:
                    break
