...
def _load_em_api_key() -> str:
    """
    加载妙想 API Key。
    依次尝试系统环境变量、仓库内 .env/.env.local、OpenClaw 默认文件。
    """
    ...
    candidate_roots = [Path.cwd(), Path(__file__).resolve().parent]
...
DEFAULT_MAX_RETRIES = max(1, int(os.environ.get("MX_STOCKS_SCREENER_MAX_RETRIES", "3") or "3"))
DEFAULT_RETRY_DELAY_SEC = max(0.5, float(os.environ.get("MX_STOCKS_SCREENER_RETRY_DELAY_SEC", "1.5") or "1.5"))
...
def _is_success_payload(payload: Dict) -> bool:
    ...
    if not isinstance(payload, Dict):
        return False
    return payload.get("code") in (0, 200, "0", "200", None) and payload.get("status") in (0, 200, "0", "200", None)
...
def _is_rate_limited_payload(payload: Dict) -> bool:
    ...
    if "操作过于频繁" in message:
        return True
    return payload.get("code") in (429, 503, "429", "503") or payload.get("status") in (429, 503, "429", "503")
...
async def query_mx_stocks_screener(
    ...
    data_node = raw.get("data") if isinstance(raw.get("data"), Dict) else {}

    if not _is_success_payload(raw):
        raw_message = str(raw.get("message") or data_node.get("message") or "请求失败").strip()
        result["error"] = raw_message
        result["raw_preview"] = raw_preview
        return result

    # 若 MCP 返回了业务错误 message，优先透传
    raw_message = str(data_node.get("message") or "").strip()
    if raw_message and raw_message not in {"成功", "success", "SUCCESS"}:
        result["error"] = raw_message
        result["raw_preview"] = raw_preview
        return result

    # 使用 datalist（全量），兼容 allResults/result 为 None 的场景
    all_results = data_node.get("allResults")
    ...
    if not rows:
        partial_results = data_node.get("partialResults")
        if partial_results:
            rows = _parse_partial_results_table(partial_results)
            data_source = "partialResults"

    if not rows:
        if data_node.get("securityCount", -1) == 0:
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            for attempt in range(1, DEFAULT_MAX_RETRIES + 1):
                ...
                if _is_rate_limited_payload(result_data) and attempt < DEFAULT_MAX_RETRIES:
