...
- 最近荐股
- 交易时间线
- 功能状态（AI Agent、资讯路由、Bark、Futu、数据库）
- 最近日志
...
- 每 30 秒刷新一次页面数据
- 自动展示数据库路径与当前实例状态
- 操作执行状态会保存在 `dashboard_cache.action_state`
...
- `dashboard_cache`
  - `market_cards`：首页指数、ETF、持仓行情卡片缓存
  - `action_state`：按钮执行状态与后台任务状态

注意事项：

- 看板本身不再展示新闻摘要
- `/api/news` 已下线
- 如果页面数据异常，优先检查当前数据库路径与 `action_state`
